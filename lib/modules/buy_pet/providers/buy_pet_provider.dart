...
import 'package:animal_market/modules/buy_pet/models/pet_near_you_model.dart';
...
class BuyPetProvider extends ChangeNotifier {
  bool isLoading = true;
  bool isLoading1 = true;
  bool noData = false;
  var buyCropList = <SubCategoryListModel>[];
  var petNearYou = <PetNearYouModel>[];
  var subCategoryList = <SubCategoryListModel>[];
  TextEditingController searchController = TextEditingController();
  Future<void> buyPetHome(BuildContext context, String categoryId) async {
    try {
      isLoading = true;
      var result = await ApiService.buyCropCattleHome(categoryId, "pet");
      ...
      if (json["status"] == true) {
        ...
        if (context.mounted) {
          if (json["data"] != null) {
            ...
            petNearYou = json["data"]["near_you"] == null ? [] : List<PetNearYouModel>.from(json['data']["near_you"].map((i) => PetNearYouModel.fromJson(i))).toList(growable: true);
          } else {
            noData = true;
          }
        }
